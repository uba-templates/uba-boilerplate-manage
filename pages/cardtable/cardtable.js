define(['text!pages/cardtable/cardtable.html', 'css!pages/cardtable/cardtable', 'pages/cardtable/cardtablemeta', 'uuigrid'], function(html) {
    var init = function(element) {
        ...
        var viewModel = {
            ...
            //定义操作列的内容
            optFun: function(obj) {
                var delfun = "data-bind=click:event.delClick.bind($data," + obj.rowIndex + ")";
                var editfun = "data-bind=click:event.editClick.bind($data," + obj.rowIndex + ")";
            ...
            event: {
                ...
                editClick: function(index) {
                    ...
                    if (index===0||index) {
                        viewModel.dtnew.setSimpleData(viewModel.dt1.getRow(index).getSimpleData());
                ...
                delClick: function(index) {
                    var row = viewModel.dt1.getSelectedRows()[0];
                    var data = viewModel.dt1.getRow(index).getSimpleData()
                    u.confirmDialog({
                        msg: "是否删除" + data.name + "?",
                        title: "删除确认",
                        onOk: function() {
                            // viewModel.event.deleteData(data);
                            viewModel.dt1.removeRow(index);
                        },
                        onCancel: function() {}
                    });
                ...
                saveOkClick: function() {
                    ...
                    if($('#code').attr("readonly")=="readonly"){
                        viewModel.dt1.getRow(index).setSimpleData(data)
                    }else {
